...
export class LightboxComponent extends BaseComponent
  ...
  constructor(differs: IterableDiffers, injector: Injector) {
    super(injector)
  ...
  plusSlides(move: number) {
    ...
    if (move < 0) {
      this.selectedImage =
        nextPos < 0 ? this.images[this.images.length - 1] : this.images[nextPos]
    } else {
      this.selectedImage =
        nextPos >= this.images.length ? this.images[0] : this.images[nextPos]
    }
  }
}
