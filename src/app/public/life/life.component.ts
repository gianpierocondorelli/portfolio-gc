  ...
  getVisibility(index: number) {
    if (this.activateVisibility && isPlatformBrowser(this.platformId)) {
      ...
      if (currentScroll <= sectionHeight * .5) {
        ...
        this.bkgSrv.sendNewImgBackground('assets/images/life/life-section-one.jpg');
        return index === 0;
      } else {
        const firstChange = currentScroll > (sectionHeight * .5);
        if (firstChange) {
          this.bkgSrv.sendNewImgBackground('assets/images/life/life-section-two.jpg');
        }
